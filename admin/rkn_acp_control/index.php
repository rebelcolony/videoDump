...
Predator CMS Admin CP Index Controller
...
class index extends rkn_render
{
	...
	public function init()
	{
		...
		{
			exit('<strong>You must be an admin to access this area</strong>');
		}
	...
	public function idx()
	{
		exit(header("Location:" . $this->rknclass->settings['site_url'] . '/' . RKN__adminpath . '/index.php?ctr=content'));
	}
	private function load_login_form()
	{
		$return=$this->rknclass->get['return_url'];
		$admin_path=RKN__adminpath; //For compatibility with heredoc syntax
		echo <<<EOD
...
<title>{$this->rknclass->settings['site_name']} :: Admin Control Panel Login</title>
...
<body>
<form action="{$this->rknclass->settings['site_url']}/$admin_path/index.php?ctr=login&amp;act=process&amp;return_url=$return" method="post" name="login">
	<div id="login" align="center">
        ...
        <div id="spacer"></div>
        <div id="links" align="center"><a class="links" href="{$this->rknclass->utils->build_url('forgot_password', 'idx')}">Forgot Password?</a></div>
	</div>
</form>
</html>		
EOD;
